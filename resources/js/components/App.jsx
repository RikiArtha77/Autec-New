...
import BecomeFarmerPage from './BecomeFarmerPage';
    ...
    <div className="flex flex-col min-h-screen bg-cover bg-center bg-no-repeat"
        ...
        <div className="space-x-6">
          ...
          {isLoggedIn && role === 'petani' && (
            <>
              ...
              <NavLink to="/sensor" className={({ isActive }) => `hover:text-gray-300 ${isActive ? 'font-bold' : ''}`}>
                SensorData
              </NavLink>
            </>
          )}
          ...
          {!isLoggedIn && (
            <>
              <NavLink to="/login" className={({ isActive }) => `hover:text-gray-300 ${isActive ? 'font-bold' : ''}`}>
                Login
              </NavLink>
              <NavLink to="/register" className={({ isActive }) => `hover:text-gray-300 ${isActive ? 'font-bold' : ''}`}>
                Register
              </NavLink>
            </>
          )}
      ...
      <main className="flex-grow mt-16 p-4">
        <Routes>
          ...
          {/* <Route path="/become-farmer" element={isLoggedIn && role === 'user' ? <BecomeFarmerPage /> : <HomePage />} /> */}
          <Route path="/device" element={isLoggedIn && role === 'petani' ? <DevicePage /> : <HomePage />} />
          <Route path="/sensor" element={isLoggedIn && role === 'petani' ? <SensorData /> : <HomePage />} />
        </Routes>
      </main>
